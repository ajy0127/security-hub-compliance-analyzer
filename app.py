...
def get_findings(hours):
    ...
    try:
        logger.info(f"Querying SecurityHub for findings between {start_time_str} and {end_time_str}")
...
def generate_csv(findings, soc2_mapper):
    ...
    # Write header
    writer.writerow([
        "Title", "Severity", "Finding Type", "SOC 2 Controls", 
        "Resource ID", "Account ID", "Region", "Description"
    ])
...
def send_email(recipient_email, findings, analysis, stats, soc2_mapper):
    ...
    msg['Subject'] = f'AWS SecurityHub SOC 2 Compliance Report - {datetime.now().strftime("%Y-%m-%d")}'
    ...
    attachment.add_header('Content-Disposition', 'attachment', filename='securityhub_soc2_findings.csv')
...
def lambda_handler(event, context):
    ...
    if event.get('test_email'):
        ...
        return {
            ...
            'body': json.dumps('Test email sent successfully' if success else 'Failed to send test email')
        }
    ...
    if not recipient_email:
        ...
        return {
            'statusCode': 500,
            'body': json.dumps('Recipient email not configured')
        }
    ...
    if not findings:
        ...
        return {
            'statusCode': 200,
            'body': json.dumps('No findings to report')
        }
    ...
    return {
        'statusCode': 200 if success else 500,
        'body': json.dumps('Email sent successfully' if success else 'Failed to send email')
    }
...
def cli_handler():
    ...
    if args.command == 'report':
        ...
        print(f"Report generated on {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} UTC\n")
        ...
        if args.csv:
            csv_data = generate_csv(findings, soc2_mapper)
            csv_path = args.csv_path or f"securityhub_soc2_findings_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"

...
I recommend prioritizing the critical finding immediately, followed by the high-severity issues within one week, and implementing a continuous monitoring solution to detect similar issues in the future. These actions would significantly improve your NIST 800-53 compliance posture."""
    ...
    mock_stats = {
        "NIST800-53": {
            ...
            "critical": sum(1 for f in mock_findings if f.get("Severity", {}).get("Label") == "CRITICAL"),
            "high": sum(1 for f in mock_findings if f.get("Severity", {}).get("Label") == "HIGH"),
            "medium": sum(1 for f in mock_findings if f.get("Severity", {}).get("Label") == "MEDIUM"),
            "low": sum(1 for f in mock_findings if f.get("Severity", {}).get("Label") == "LOW")
        }
    }

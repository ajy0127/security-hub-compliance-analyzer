    ...
    return {
        "body": MagicMock(
            read=lambda: json.dumps(
                {
                    "messages": [
                        {"role": "assistant", "content": "Test analysis summary"}
                    ]
                }
            )
        )
...
def test_lambda_handler_success(sample_findings, mock_bedrock_response, mock_env):
    with patch("boto3.client") as mock_boto3:
        ...
        assert "findingsAnalyzed" in body
...
def test_lambda_handler_no_findings(mock_env):
    with patch("boto3.client") as mock_boto3:
        ...
        body = json.loads(response["body"])
        assert "findingsAnalyzed" in body
        assert body["findingsAnalyzed"] == 0
...
def test_lambda_handler_test_email(mock_env):
    with patch("boto3.client") as mock_boto3:
        ...
        response = lambda_handler(
            {"test_email": True, "recipient_email": "test@example.com"}, {}
        )
        assert response["statusCode"] == 200
        assert "message_id" in json.loads(response["body"])

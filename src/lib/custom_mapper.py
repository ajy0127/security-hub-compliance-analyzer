...
class CustomControlMapper(SOC2Mapper):
    ...
    def __init__(self, custom_config_path: Optional[str] = None):
        ...
        org_config_path = os.environ.get(
            ...
            str(Path(__file__).parent.parent.parent / "config" / "custom_controls.json")
        )
        ...
        try:
            if Path(org_config_path).exists():
                with open(org_config_path, "r") as f:
                    ...
                    logger.info(
                        f"Loaded organization-specific config",
    ...
    def map_finding_to_controls(self, finding: Dict[str, Any]) -> Dict[str, List[str]]:
        ...
        mapped_controls["secondary_controls"] = [
            c for c in mapped_controls["secondary_controls"] 
            if c not in mapped_controls["primary_controls"]
        ]
        
        return mapped_controls
    
    def _apply_regex_mappings(self, finding: Dict[str, Any], mapped_controls: Dict[str, List[str]]) -> None:
        """
        ...
        for pattern, mapping in self.regex_mappings.items():
            try:
                ...
                if (re.search(pattern, finding_type, re.IGNORECASE) or
                    re.search(pattern, finding_title, re.IGNORECASE) or
    ...
    def _apply_resource_mappings(self, finding: Dict[str, Any], mapped_controls: Dict[str, List[str]]) -> None:
        """

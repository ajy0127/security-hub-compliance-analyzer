...
class SOC2Mapper:
    def __init__(self):
        ...
        """
        config_path = Path(__file__).parent.parent.parent / 'config' / 'soc2_control_mappings.json'
        try:
    ...
    def get_control_description(self, control_id):
        ...
        """
        return self.mappings['control_descriptions'].get(control_id, 'Description not available')
    ...
    def format_finding_for_soc2(self, finding):
        ...
        for control in mapped_controls['primary_controls']:
            soc2_finding = {
                ...
                'Remediation_Steps': finding.get('Remediation', {}).get('Recommendation', {}).get('Text', 'No remediation steps provided'),
                'Remediation_Timeline': '30 days' if risk_level == 'High' else '90 days' if risk_level == 'Medium' else '180 days',
                'Evidence_Reference': finding.get('ProductArn', 'N/A'),
                'Audit_Impact': f"This finding affects compliance with {control} and requires immediate attention" if risk_level == 'High' else f"This finding affects compliance with {control}",

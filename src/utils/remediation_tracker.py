...
class RemediationTracker:
    ...
    def get_prior_finding_status(self, finding_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error(f"Error retrieving finding status: {str(e)}", finding_id=finding_id)
            return None
    ...
    def update_finding_status(self, finding: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if prior_status:
                ...
                # Calculate days open
                first_seen_date = datetime.fromisoformat(item['first_seen'].replace('Z', '+00:00'))
            ...
            logger.error(f"Error updating finding status: {str(e)}", finding_id=finding_id)
    ...
    def mark_as_remediated(self, finding_id: str, notes: Optional[str] = None) -> None:
        ...
        try:
            ...
            prior_status = self.get_prior_finding_status(finding_id)
            if not prior_status:
                logger.warning(f"Cannot mark unknown finding as remediated", finding_id=finding_id)
                return
            ...
            first_seen_date = datetime.fromisoformat(prior_status['first_seen'].replace('Z', '+00:00'))
            now_date = datetime.fromisoformat(now.replace('Z', '+00:00'))
            ...
            logger.info(
                f"Marked finding as remediated",
                finding_id=finding_id,
                remediation_days=remediation_days
            )
            
        except Exception as e:
            logger.error(f"Error marking finding as remediated: {str(e)}", finding_id=finding_id)
    ...
    def get_remediation_status(self, findings: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        remediated_finding_ids = {
            item['finding_id'] for item in all_tracked_items
            if item['current_status'] != 'REMEDIATED' and item['finding_id'] not in current_finding_ids
        }
        ...
        for item in all_tracked_items:
            if item['finding_id'] in ongoing_finding_ids:
                ...
                age_stats[severity]['avg_days'] = (
                    (age_stats[severity]['avg_days'] * (age_stats[severity]['count'] - 1) + days_open) / 
                    age_stats[severity]['count']
                )
                age_stats[severity]['max_days'] = max(age_stats[severity]['max_days'], days_open)
        ...
        for finding_id in remediated_finding_ids:
            self.mark_as_remediated(finding_id, "Automatically marked as remediated - no longer found")
        ...
        return {
            'new_findings': {
                ...
                'findings': [{'id': f.get('Id'), 'title': f.get('Title')} for f in new_findings]
            },
            'remediated_findings': {
            ...
            'ongoing_findings': {
                ...
                'findings': [{'id': f.get('Id'), 'title': f.get('Title')} for f in ongoing_findings]
            },
            'age_statistics': age_stats,
            'total_active': len(findings),
            'total_remediated': len(remediated_finding_ids),
            'remediation_rate': len(remediated_finding_ids) / max(1, len(findings) + len(remediated_finding_ids))
        }
    ...
    def get_finding_history(self, finding_id: str) -> Dict[str, Any]:
        ...
        if not item:
            return {'finding_id': finding_id, 'status': 'UNKNOWN', 'message': 'No history found'}

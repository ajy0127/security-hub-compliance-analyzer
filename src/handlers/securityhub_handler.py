...
def summarize_findings(findings):
    ...
    for f in selected_findings:
        ...
        finding_summary = {
            ...
            'Description': f.get('Description', 'N/A')[:200] + '...' if len(f.get('Description', '')) > 200 else f.get('Description', 'N/A'),
            'SOC2_Controls': {
...
def lambda_handler(event, context):
    ...
    try:
        ...
        if not sender_email or not recipient_email:
            raise ValueError("Missing required environment variables: SENDER_EMAIL and RECIPIENT_EMAIL")
        ...
        findings_response = security_hub.get_findings(
            Filters={
                'CreatedAt': [{'Start': start_time.isoformat(), 'End': datetime.now().isoformat()}],
        ...
        if not findings:
            ...
            return {
                'statusCode': 200,
                'body': json.dumps('No findings to analyze')
            }
        ...
        # Create email
        msg = MIMEMultipart()
        msg['Subject'] = f'SecurityHub SOC 2 Compliance Report - {datetime.now().strftime("%Y-%m-%d")}'
